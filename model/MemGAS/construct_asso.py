...
def construct_asso(dataset, work_path, retriever='contriever'):    
    
    in_data = json.load(open(work_path + f'/data/process_data/{dataset}.json'))
    all_emb = torch.load(work_path + f'/data/process_embs/{dataset}-{retriever}-emb.pt')
    if os.path.exists(work_path + f'/data/process_embs/{dataset}-{retriever}-emb.pt'):
        all_emb = torch.load(work_path + f'/data/process_embs/{dataset}-{retriever}-emb.pt')
    else:
        all_emb = emb_rawdata(dataset, work_path, retriever)
    ...
    for entry, emb in tqdm(zip(in_data,all_emb), total=min(len(in_data), len(all_emb))):
        ...
        for i in range(5*4,len(node_ids)):
            ...
            edges = get_edges(cur_mem, i, pre_mem, 20, 2) # currently hard code
            all_edges += edges
    ...
    torch.save(covid2graph, work_path+f"/data/graph_cache/graph-{dataset}-{retriever}.pt")
    return covid2graph
