...
class EmbeddingModelContriever():
    def __init__(self):
        ...
        self.model = AutoModel.from_pretrained('facebook/contriever').to(torch.device('cuda'))
...
def emb_rawdata(dataset, work_path, retriever):
    os.makedirs(work_path+'/data/process_embs/', exist_ok=True)
    data_path=work_path + f'/data/process_data/{dataset}.json'
    save_path=work_path + f'/data/process_embs/{dataset}-{retriever}-emb.pt'
    in_data = json.load(open(data_path))
    ids2summary = read_ids2granularity(work_path+f'/data/multi_granularity_logs/{dataset}-summary_level.jsonl')
    ids2keyword = read_ids2granularity(work_path+f'/data/multi_granularity_logs/{dataset}-keyword_level.jsonl')
